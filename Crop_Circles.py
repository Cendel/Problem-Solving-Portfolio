# Question: https://www.codingame.com/ide/puzzle/crop-circles
    ...
    for j in range(a):
        for y in range(columns[c]-(d//2*2), columns[c]+(d//2*2)+1, 2):
            if columns["a"] <= y <= columns["s"] and rows[r]+j-b <= rows["y"]:
                grid[rows[r]+j -
                     b][y] = " " if do == "MOW" else "{" if do == "PLANT" else " " if do == "reverse" and grid[rows[r]-b-j][y] == "{" else "{"
    ...
    for j in range(1, z+1):
        for y in range(columns[c]-(d//2*2)+j*2, columns[c]+(d//2*2)-j*2+1, 2):
            if columns["a"] <= y <= columns["s"] and rows[r]-b-j >= rows["a"] and rows[r] + b + j <= rows["y"]:
                grid[rows[r]-b -
                     j][y] = " " if do == "MOW" else "{" if do == "PLANT" else " " if do == "reverse" and grid[rows[r]-b-j][y] == "{" else "{"
